    ...
    vm.getCarteiras = () => {
        ...
        globalService.Call('p_dependencias_jurisdicionadas', parametro, response => {
            $rootScope.$broadcast('waitScreen', false);
             if (response.success) {
                 vm.item = []
                 // Mostra no console quantos dados possui.
                 var teste = response.data
                 console.log(teste.length)
                 // A partir de 56 não vem mais nulo. E percorre o for até a quantidade máxima (5982)
                 // i < 5982 (númedo atual) ou caso criem mais o for virá com o tamanho do array no caso teste.length
                 // Retorna todos nomes: nome carteira, primeiro nome, nome completo do responsável
                 for (var i = 56; i < teste.length; i++) {
                    vm.item.push(response.data[i].nome_carteira)
                    vm.item.push(response.data[i].nome_xx)
                    vm.item.push(response.data[i].nome_resp)
                 }
